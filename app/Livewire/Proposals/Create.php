...
use App\Notifications\BetterProposal;
...
class Create extends Component
{
    ...
    public function save()
    {
        ...
        $this->project->author(new NewProposal($this->project));
    ...
    {
        $query = DB::select(
            'select *, row_number() over(order by hours asc) 
            as newPosition from proposals where project_id = :project',
        ...
        $otherProposal = collect($query)
            ->where('position', '=', $position->newPosition)->first();

        if ($otherProposal) {
            $proposal->update(['position_status' => 'up']);
            $oProposal = Proposal::find($otherProposal->id);
            $oProposal->update(['position_status' => 'down']);
            $oProposal->notify(new BetterProposal($this->project));
        }
